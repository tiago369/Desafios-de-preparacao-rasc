...
int main() {
  ...
  double distance, speed_modifier, sensor_value, lumin;
  ...
  while (wb_robot_step(time_step) != -1) {
    ...
    lumin = wb_light_sensor_get_value(light_sensor);
    printf("%f\n", lumin);
    if(lumin > 800){
      state = STOP;
    }
    ...
    for (i = 0; i < MAX_SENSOR_NUMBER; ++i) {
      ...
      if (sensor_value == 0.0)
        ...
        distance = 5.0 * (1.0 - (sensor_value / MAX_SENSOR_VALUE));  // lookup table inverse. _____OBSERVAR VALOR_____
